    ...
    <view class="wrapper">
      <view class="toolbar" @tap="format">
        ...
        <view class="text_color">
          <text :style="`backgroundColor:${item1}`" v-for="(item1, index1) of choice_color_list" :key="index1" @click="change_text_color(index1)"></text>
        </view>
        <!-- 选着背景颜色面板 -->
        <view class="bg_color">
          <text :style="`backgroundColor:${item1}`" v-for="(item1, index1) of choice_color_list" :key="index1" @click="change_bg_color(index1)"></text>
        </view>
        <!-- 文字颜色 -->
        <view
        ...
        <!-- 调整字体大小 -->
        <view :class="formats.fontSize === '24px' ? 'ql-active' : ''" class="iconfont icon-fontsize" data-name="fontSize" data-value="24px" @tap.stop="change_font">
          <!-- 字号大小选择器 -->
          <view class="fontSize">
        ...
        <!-- H1 -->
        <view :class="formats.header === 1 ? 'ql-active' : ''" class="iconfont icon-format-header-1" data-name="header" :data-value="1" @tap.stop="trade_name">
          <!-- 字号大小选择器 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fillStyle(styleObj, obj = {}) {
        for (let key in styleObj) {
          let item = styleObj[key];
          if (typeof item !== 'undefined') {
            obj[key] = typeof item === 'number' ? `${item}px` : item;
          }
        }
        return obj;
      },
    ...
    computed: {
      styleCom() {
        let { width } = this;
        return this.fillStyle({ width });
      },
      editorStyleCom() {
        let { minHeight, maxHeight } = this;
        return this.fillStyle({ minHeight, maxHeight });
      },
    },
  };
</script>

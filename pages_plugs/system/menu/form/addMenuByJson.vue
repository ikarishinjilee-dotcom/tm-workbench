...
<script>
  ...
  const normalizeJsonSymbols = (value) => {
    if (typeof value !== 'string') return value;
    return value.replace(/[【】［］｛｝（），、：；“”＂「」『』﹁﹂﹃﹄‘’＇？！。]/g, (symbol) => jsonSymbolMap[symbol] || symbol);
  };
  export default {
    props: {
      value: {
          ...
          props: {
            ...
            columns: [
              ...
              },
              {
                ...
                watch: ({ value, $set }) => {
                  const menus = normalizeJsonSymbols(value);
                  if (menus !== value) {
                    $set('menus', menus);
                  }
                },
            ...
            // 表单验证规则
            rules: {
              menus: [{ required: true, message: '不能为空', trigger: 'change' }],
            },
            ...
            beforeAction(formData) {
              try {
                let menus = JSON.parse(normalizeJsonSymbols(formData.menus));
    ...
    methods: {
      ...
      onOpen() {
        ...
        this.form1.data.parent_id = menu_id;
      },
      // 监听 - 页面关闭
      onClose() {

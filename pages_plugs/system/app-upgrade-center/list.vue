  ...
  <div v-loading="loading">
    ...
    <div class="vk-page-card">
      ...
      <div class="vk-page-card-toolbar">
        <div class="vk-page-card-title">版本列表</div>
        <div class="vk-page-card-actions">
          <el-dropdown placement="bottom-start" @command="addBtn">
            <el-button type="primary" :size="$global.size" icon="el-icon-plus" :disabled="!queryForm1.formData.appid">添加版本</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init(options) {
        ...
        vk.callFunction({
          ...
          data: {
            ...
            columns: [{ key: 'enable_upgrade_center', type: 'select', mode: '=' }],
          },
          success: (data) => {
            this.appList = data.rows;
            if (options.appid) {
              this.queryForm1.formData.appid = options.appid;
              this.search();
            } else if (data.rows.length > 0) {
              this.queryForm1.formData.appid = data.rows[0].appid;
              this.search();
            }
          },
        });
      ...
      // 搜索
      search() {
        if (this.$refs.table1) this.$refs.table1.search();
      },
      // 监听 - 查询表单折叠面板变化事件
      collapseChange() {
        // 执行表格重新布局
        if (this.$refs.table1) this.$refs.table1.doLayout();
      },
      // 刷新
      refresh() {
      ...
      // 显示添加页面
      addBtn(type) {
        let appid = this.queryForm1.formData.appid;
        let appInfo = vk.pubfn.getListItem(this.appList, 'appid', appid);
        let item = {
          appid,
          type,
          name: appInfo.name,

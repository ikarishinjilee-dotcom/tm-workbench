<template>
  <vk-data-dialog v-model="value.show" :title="page.title" :top="page.top" :width="page.width" :close-on-click-modal="true" mode="form" @open="onOpen" @closed="onClose">
    <!-- 页面主体内容开始 -->
    <vk-data-form
      ref="form1"
      v-model="form1.data"
  ...
  export default {
              ...
              if (!formData.login_appid_type) {
                ...
                if (typeof formData.dcloud_appid === 'object') {
                  formData.dcloud_appid.map((appid, index) => {
                    let item = vk.pubfn.getListItem(this.value.item.dcloudAppidData, 'appid', appid);
                    if (item.type === 'admin') {
                      allow_login_background = true;
                    }
                  });
                } else {
                  let item = vk.pubfn.getListItem(this.value.item.dcloudAppidData, 'appid', formData.dcloud_appid);
                  if (item.type === 'admin') {
                    allow_login_background = true;
                  }
                }
              }
              formData.user_ids = this.value.item.user_ids;
    ...
    methods: {
      ...
      onOpen() {
        ...
        this.form1.props.columns[2].data = item.dcloudAppidData;
        this.form1.props.columns[3].data = item.dcloudAppidData;
      },
      // 监听 - 页面关闭
      onClose() {

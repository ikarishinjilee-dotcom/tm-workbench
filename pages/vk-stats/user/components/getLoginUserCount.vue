<template>
  <el-row>
    <el-col :xs="24" :md="12" :lg="6">
      <el-card class="box-card" v-loading="loading">
        <view slot="header" class="header">
          <text class="header-title">今日登录用户</text>
        ...
        <view class="box-card-contrast">
          ...
          <view>
            ...
            <text class="value" :class="contrastFn(data.today, data.day7Total / 7).symbol === '+' ? 'up' : 'down'">{{ contrastFn(data.today, data.day7Total / 7).value }}</text>
          </view>
        </view>
      </el-card>
    </el-col>
    <el-col :xs="24" :md="12" :lg="6">
      <el-card class="box-card" v-loading="loading">
        <view slot="header" class="header">
          <text class="header-title">昨日登录用户</text>
        ...
        <view class="box-card-contrast">
          ...
          <view>
            <text class="label ml10">较7日平均</text>
            <text class="value" :class="contrastFn(data.yesterday, data.day7Total / 7).symbol === '+' ? 'up' : 'down'">{{
              contrastFn(data.yesterday, data.day7Total / 7).value
            }}</text>
          </view>
        </view>
      </el-card>
    ...
    <el-col :xs="24" :md="12" :lg="6">
      <el-card class="box-card" v-loading="loading">
        <view slot="header" class="header">
          <text class="header-title">近7日登录用户</text>
    ...
    <el-col :xs="24" :md="12" :lg="6">
      <el-card class="box-card" v-loading="loading">
        ...
        <view class="box-card-contrast">
          <view>
            <text class="label">本月登录</text>
            <text class="value up">{{ data.thisMonth }}人</text>
          </view>
          <view>
            <text class="label ml10">本年登录</text>
...
<script>
  export default {
    ...
    data() {
      return {
        url: 'admin/system_uni/sys.stats.getLoginUserCount',
        data: {
          all: '-', // 总
          today: '-', // 今日
          yesterday: '-', // 昨日
